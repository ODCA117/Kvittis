...
async fn main() -> Result<()> {
    ...
    let RegisterResponse {
        id: alice_id,
        username: alice_un,
    } = client.register_user("alice").await?;
    let RegisterResponse {
        id: bob_id,
        username: bob_un,
    } = client.register_user("bob").await?;
    let RegisterResponse {
        id: charlie_id,
        username: charlie_un,
    } = client.register_user("charlie").await?;
    ...
    let CreateGroupResponse {
        ...
        .create_group("family", alice_id, vec![alice_id, bob_id])
        .await?;
    let CreateGroupResponse {
        id: friend_id,
        name: friend_name,
    } = client
        .create_group("friends", charlie_id, vec![alice_id, charlie_id])
        .await?;
